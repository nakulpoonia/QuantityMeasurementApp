...
class Quantity<U extends IMeasurable>
{
    ...
    public Quantity<U> subtract(Quantity<U> oth)
    {
        double b1 = unit.convertToBaseUnit(val);
        double b2 = oth.unit.convertToBaseUnit(oth.val);

        double sub = b1 - b2;

        double ans = unit.convertFromBaseUnit(sub);
        return new Quantity<>(ans, unit);
    }

    public Quantity<U> subtract(Quantity<U> oth,U target)
    {
        double b1 = unit.convertToBaseUnit(val);
        double b2 = oth.unit.convertToBaseUnit(oth.val);
        ...
        double ans = target.convertFromBaseUnit(sub);

        return new Quantity<>(ans, target);
    }


    public double divide(Quantity<U> oth)
    {
        double b1 = unit.convertToBaseUnit(val);
        double b2 = oth.unit.convertToBaseUnit(oth.val);
